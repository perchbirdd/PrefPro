...
public partial class PluginUI : IDisposable
{
    [GeneratedRegex("[^A-Za-z'\\-\\s{1}]")]
    ...
    private string _tmpFirstName = string.Empty;
    private string _tmpLastName = string.Empty;
    ...
    private string SanitizeName(string first, string last)
    {
        ...
        var lastValid = _configuration.Name;

        if (newFirst.Length > 15 || newLast.Length > 15)
            return lastValid;

        var combined = $"{newFirst}{newLast}";
        if (combined.Length > 20)
            return lastValid;

        newFirst = CleanNameRegex().Replace(newFirst, "");
        newLast = CleanNameRegex().Replace(newLast, "");
